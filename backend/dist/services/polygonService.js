...
const polygon_url = process.env.POLYGON_BASE_URL?.replace(/\/$/, "") ??
    "https://api.polygon.io";
const apiKey = process.env.POLY_API_KEY;
const MAX_REQUESTS_PER_MIN = Number(process.env.POLY_MAX_RPM ?? 40) || 40;
...
export const getLastTrade = async (symbol) => {
    ...
    return cacheGetOrSet(key, async () => {
        if (!canRequest())
            return null;
        const { data } = await axios.get(`${polygon_url}/v2/last/trade/${symbol}?apiKey=${apiKey}`);
...
export const getLastTradeSafe = async (symbol) => {
    try {
